...
* **商业智能与可视化**：时序同群组分析 (Cohort Analysis), 关联规则协同矩阵 (Co-occurrence Matrix), ipywidgets 交互式看板
...
│   ├── 02_funnel_analysis.ipynb      # EDA、假设检验与多变量统计
│   ├── 03_rfm_segmentation.ipynb     # 无监督聚类与 XGBoost 分类器实验
│   └── 04_dashboard.ipynb            # 基于 ipywidgets 的交互式前端 BI 看板
├── src/
│   ├── cleaner.py           # 数据预处理模块
│   ├── funnel.py            # 漏斗、同群组与关联矩阵计算模块
│   ├── rfm.py               # 聚类与预测建模封装模块
